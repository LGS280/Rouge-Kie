...
public class RookieHealth : MonoBehaviour
{
    ...
    {
        if (currentMana < amount) return false; // không đủ mana
        currentMana -= amount;
        onHealthChanged?.Invoke();
        return true;
    }
    ...
    void Update()
    {
        ...
        {
            TakeDamage(1);
            Debug.Log("HP: " + GetCurrentHealth() + "/" + GetMaxHealth() + " | Armor: " + currentArmor + "/" + maxArmor);
        }
        ...
        if (currentArmor < maxArmor && !isDead)
        {
            ...
            {
                // Đang chờ 2 giây trigger
                armorRegenDelayTimer -= Time.deltaTime;
                if (armorRegenDelayTimer <= 0f)
                {
                    armorRegenStarted = true;
                    armorRegenTickTimer = armorRegenTick; // bắt đầu đếm tick đầu tiên
                }
            }
            else
            {
                // Đang hồi từng giáp mỗi 1 giây
                armorRegenTickTimer -= Time.deltaTime;
                if (armorRegenTickTimer <= 0f)
                {
                    currentArmor++;
                    onHealthChanged?.Invoke();
                    armorRegenTickTimer = armorRegenTick; // reset tick

                    if (currentArmor >= maxArmor)
                        armorRegenStarted = false; // dừng khi đầy
                }
            }
        }
    }
    ...
    public void TakeDamage(int damage)
    {
        ...
        // Reset hoàn toàn timer khi bị đánh
